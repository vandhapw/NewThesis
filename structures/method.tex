...
\begin{enumerate}
    ...
    \item A quantum risk encoding function $\Gamma: \mathbb{R}^{5} \to [-1, +1]$ implemented via Pauli-Z multi-sensor encoding that maps a machine's five normalized sensor values to a composite quantum expectation value $\langle Z \rangle_c \in [-1, +1]$ and a quantum risk label 
$\ell \in \{
...
\subsubsection{Layer 4: Quantum Computing Enhancement}
...
\begin{figure}[htbp]
    ...
    \caption{Integrated Quantum-Agentic architecture: The Quantum Layer (Pauli-Z encoding, QUBO annealing, QAOA scheduling) runs once per machine batch and feeds a consolidated quantum context into the per-machine Agentic Decision Loop. The \texttt{should\_quantum\_override} gate escalates machines with quantum danger signals regardless of classical risk level. The Learning Loop at the bottom implements Q-FN feedback and threshold adaptation.}
    \label{fig:quantum_agent_architecture}
\end{figure}

\paragraph{Pauli-Z Multi-Sensor Risk Encoding.} The \texttt{PauliZRiskEncoder} class constructs a 5-qubit circuit for each machine: Hadamard gates create superposition; parameterized RZ gates encode normalized sensor values as rotation angles $\theta_i = \pi \cdot \text{norm}_i$; CNOT gates between adjacent qubits introduce entanglement capturing pairwise sensor correlations. The per-sensor expectation is $\langle Z_i \rangle = \cos(\pi \cdot \text{norm}_i)$, yielding $+1$ for safe sensors, $0$ for marginal sensors, and $-1$ for sensors in the danger zone. The weighted composite $\langle Z \rangle_c = \sum_i w_i \langle Z_i \rangle$ (weights: temperature 0.30, vibration 0.30, humidity 0.15, pressure 0.15, energy 0.10) maps to quantum risk label \texttt{QUANTUM-ANOMALY} ($\langle Z \rangle_c < -0.20$), \texttt{QUANTUM-WARN} ($-0.20 \leq \langle Z \rangle_c < 0.30$), \texttt{QUANTUM-MARGINAL} ($0.30 \leq \langle Z \rangle_c < 0.60$), or \texttt{QUANTUM-NORMAL} ($\geq 0.60$).

\paragraph{Quantum Annealing for QUBO Priority Ranking.} The \texttt{QuantumAnnealingOptimizer} class formulates machine prioritization as a QUBO problem: the matrix $Q$ encodes the objective of selecting highest-risk machines (diagonal terms $-w_i$) with a cardinality penalty (off-diagonal terms $+\lambda/2$). Simulated annealing with 500 iterations and exponential cooling (from $T = 5.0$ to $T_{\min} = 10^{-4}$) yields a binary selection vector and full priority ranking of all machines by quantum risk score. The optimal QUBO energy is logged as a convergence quality measure.

\paragraph{QAOA Time-Slot Scheduling.} The \texttt{QAOAScheduler} class assigns all machines to maintenance time slots using QAOA. The problem Hamiltonian is $H = \frac{1}{2}\sum_{(i,j) \in E}(I - Z_i Z_j)$ augmented with Pauli-Z bias terms $-r_i \cdot 0.3 \cdot Z_i$ that steer high-risk machines toward Slot~A. The \texttt{QAOAAnsatz} with $p = 2$ repetitions is optimized via COBYLA (150 iterations) using the Qiskit Aer \texttt{EstimatorV2} primitive. The optimal bitstring from 1024 measurement shots determines Slot~A (immediate/urgent) and Slot~B (planned/routine) assignments for all machines.
