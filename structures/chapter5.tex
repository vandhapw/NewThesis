...
To verify that the orchestrator scales to a realistic fleet size and to characterise the handover from the cognitive layer to the scheduling layer, the full streaming pipeline was executed on $N=50$ simulated machines, each fed 60 sensor readings split into 5 analysis windows of 10 readings. The constraints engine, priority computation, and routing to the scheduling solver are exactly those described in Sections~5.3 and~5.4; no agent-side parameter was retuned for this experiment.

\subsection{Streaming Outcome Summary}
Table~\ref{tab:stream50_summary} summarises the cognitive-layer output. After processing 5 windows per machine, the constraints engine flagged 45 of 50 machines as requiring maintenance, all of them at the CRITICAL priority level; the remaining 5 machines were classified LOW and excluded from the scheduling request. The average priority score across the 45 forwarded machines is 144.4, the average health index is 0.479, and the average damage index is 0.438, reflecting the cumulative effect of repeated constraint violations across the 50-reading horizon.
...
The 45 forwarded machines define a QUBO instance with $T=19$ time slots (slot size $\approx 3$ hours over the 72-hour horizon) and $n_{\text{bits}} = N_{\text{maint}} \cdot T = 855$ binary decision variables. This is a substantially larger instance than the synthetic benchmarks of Chapter~6 (which top out at $n=390$ for $N=30$). Table~\ref{tab:stream50_solvers} reports the solver outcomes when this instance is handed to the same MILP, QIEA, and SQA solvers used in Chapter~6.
...
The result of this handover is informative for the overall framework: at 50 machines, MILP returns the exact optimal schedule in $\sim$37~ms, comfortably inside the 60-s Kafka cycle budget established in Chapter~4. QIEA terminates in $\sim$12~s with a numerically lower cost ($7{,}958.7$ versus $21{,}455.6$) but the lower cost is itself the symptom of infeasibility: the solver's search has lapsed into a region of the $\{0,1\}^{855}$ space in which the one-hot assignment penalty no longer dominates, so a nominal QUBO energy can be reached without committing every machine to exactly one time slot. SQA does not return a result within its solver budget at this scale. We therefore route the scheduling layer to MILP/CBC by default for $N \geq 30$, while retaining QIEA and SQA for the smaller-fleet regime characterised in Chapter~6.
