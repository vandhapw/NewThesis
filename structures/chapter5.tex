...
\textbf{Tier~1 --- physical-state constraints} (this section, Table~\ref{tab:sensor-model-constraint}) live in \texttt{Constraints.txt} and the \texttt{CONSTRAINTS} dictionary of \texttt{config.py}. They are populated from three sources: (i) machine-vendor datasheets that specify nameplate operating ranges and critical thresholds, (ii) historical sensor distributions from the $100{,}000$-reading reference dataset of Chapter~4 used to calibrate \texttt{T\_ref}, \texttt{V\_ref}, \texttt{H\_ref}, \texttt{P\_ref}, and \texttt{E\_ref}, and (iii) classical reliability literature \cite{mobley_introduction_2002,jardine_review_2006} for trend-and-derivative thresholds, cavitation envelopes, and damage-decay constants. The \texttt{ConstraintsEngine.evaluate()} method consumes these parameters with no other configuration; substituting a different machine fleet only requires updating the dictionary.

\textbf{Tier~2 --- scheduling constraints} (Chapter~6, Constraints (C1)--(C4)) live in \texttt{SCHEDULING\_CONFIG} and \texttt{CONSTRAINTS} of the same \texttt{config.py}. \emph{(C1) one-hot} reflects the operational rule that a maintenance task is performed once per planning horizon per machine. \emph{(C2) slot-window capacity} ($C=3$ machines per active maintenance window) is set by the operator-defined crew availability and spare-parts policy. \emph{(C3) risk horizon} ($24$~h) is the operator-mandated deadline for any high-priority machine, derived from the historical mean-time-to-failure of cumulative-damage trajectories that exceed the danger threshold. \emph{(C4) must-schedule} is the closure rule that every machine flagged \texttt{needs\_maintenance} by Tier~1 enters the (C1) decision variables. The four scheduling constraints are not ML-tuned; they are operator-set integers that the scheduling layer of Chapter~6 must satisfy as hard constraints (MILP linear inequalities) or as penalty terms (QUBO), with the same numeric values across all solver paradigms to keep the comparison fair.
...
The $45$ forwarded machines yield a QUBO with $T=19$ slots ($\sim 3$~h each) and $n_{\text{bits}} = 855$ binary variables — substantially larger than the $n=390$ synthetic ceiling at $N=30$ in Chapter~6. Table~\ref{tab:stream50_solvers} reports MILP/QIEA/SQA outcomes on this instance.

\begin{table}[!htbp]
\centering
\caption{Solver outcomes on the 50-machine streaming instance ($N_{\text{maint}}=45$, $T=19$, $n_{\text{bits}}=855$). Numerics from \texttt{streaming\_50machines\_results.json}.}
\label{tab:stream50_solvers}
\small
\renewcommand{\arraystretch}{1.05}
\setlength{\tabcolsep}{4pt}
\begin{tabularx}{\textwidth}{
...
\toprule
\textbf{Solver} & \textbf{Pure cost} & \textbf{Feasible} & \textbf{Wall time} & \textbf{Notes} \\
\midrule
MILP / CBC & $21{,}455.6$ & Yes (45/45) & 36.9~ms     & exact optimum, all forwarded machines scheduled \\
QIEA       & $7{,}958.7$  & No          & 12{,}447.7~ms & lower nominal cost masks one-hot violations \\
SQA        & ---          & ---         & timeout      & no result returned within solver budget \\
\bottomrule
\end{tabularx}
\end{table}
\FloatBarrier

At $N=50$, MILP returns the exact optimum in $\sim 37$~ms, comfortably within the 60-s Kafka cycle. QIEA terminates in $\sim 12$~s with a misleadingly lower cost ($7{,}958.7$ vs $21{,}455.6$): the lower number is itself the infeasibility signature — the search drifts into a $\{0,1\}^{855}$ region where the one-hot penalty no longer dominates, so QUBO energy drops without committing every machine. SQA times out. We route scheduling to MILP/CBC by default for $N\geq 30$, retaining QIEA/SQA only for the small-fleet regime of Chapter~6.

This single end-to-end run also serves as an integration test of the multi-agent system: the per-window dispatch (Algorithm~\ref{alg:stream_processing}), the four-phase orchestration (Algorithm~\ref{alg:multi_agent}), the priority computation, and the QUBO construction (Algorithm~\ref{alg:build_qubo}, Chapter~6) are all exercised in series for 50 machines without manual intervention, establishing that the system as a whole closes the loop from raw Kafka events to a constraint-satisfying maintenance schedule under realistic fleet conditions.