...
direct message passing. Each agent reads from and writes to this shared state under orchestrator coordination. The communication protocol is defined by the \texttt{AgentMessage} record, whose fields are summarised in Table~\ref{tab:agentmessage}.
\begin{table}[!htbp]
\centering
...
to the agent pipeline. The non-blocking consumer loop --- which polls Kafka, appends each parsed reading to its per-machine buffer, and dispatches a full window to a worker-thread pool without blocking further ingestion --- is given in Algorithm~\ref{alg:stream_processing} (Appendix~\ref{app:algorithms}).
...
The artefact emitted by the Phase~1--2 agents (Monitoring + Correlation + Pattern Learning $\to$ Diagnosis $\to$ Planning) for each machine in the streaming run is a structured per-machine state record consumed by the Phase~3 Action agent and forwarded to the scheduling layer. Table~\ref{tab:agent_state_example} reproduces the record for \texttt{machine\_002} from the 50-machine streaming run; the complete fleet output is the list of $45$ such records that defines the scheduling-layer input.
