...
\begin{figure}[!htbp]
    ...
    \caption{Four-panel model performance comparison: (top-left) $R^2$ score per sensor feature for all three architectures; (top-right) MSE per feature on log scale; (bottom-left) aggregate average $R^2$, inverse MSE, and inverse MAE; (bottom-right) training loss convergence over 15 epochs. T-GCN (CC-Adj, gold) consistently outperforms both the OptimizedLSTM (teal) and Hybrid LSTM-GCN (magenta) baselines and achieves the fastest convergence to the lowest final training loss.}
...
\begin{figure}[!htbp]
    ...
    \caption{Prediction vs.\ Actual sample comparison for 200 test samples across temperature, vibration, humidity, and pressure channels. Each panel overlays actual values (solid black) with predictions from OptimizedLSTM, T-GCN (CC-Adj), and Hybrid LSTM-GCN. Per-model $R^2$ values are annotated in each legend. T-GCN consistently achieves the closest alignment with actual trajectories and the highest $R^2$ across all four sensor channels.}
...
\subsection{Anchor Strategy: Machine 40 as the Sensing Surrogate}
...
\begin{verbatim}
 ...
 |  5 Sensor Active   |          |  Partial/No Sensor     |
 +--------+---------+          +-----------+------------+
          |                                |
          v                                v
 +--------+---------+          +-----------+------------+
 ...
 +--------+---------+          +-----------+------------+
          ...
          +------------------->|  (edge from Machine 40)|
                    ...
                    v                                             v
         +----------+----------+                       +----------+----------+
         | Prediction Machine 40 |                       | Prediction Machine i|
         | (next-step forecast)  |                       | (influenced by    |
         +-----------------------+                       |  Machine 40 data)  |
                                                         +---------------------+
\end{verbatim}
\end{minipage}
