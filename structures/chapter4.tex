...
\end{equation}
where $\mathbf{W}^{(l)} \in \mathbb{R}^{d_l \times d_{l+1}}$ is the trainable weight matrix, and $\sigma(\cdot)$ denotes the ReLU activation function. A dropout rate of 0.2 is applied between GCN layers for regularization.
...
\input{figures/tgcn-architecture}
...
\subsection{The Sensor Domain Mismatch Problem}
