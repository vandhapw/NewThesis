...
Beyond static graph analysis, the T-GCN enables a practical deployment scenario in which Machine~40 equipped with a fully active sensor suite---serves as the sole anchor data source for the entire machine pool. In this configuration, the Anchor Buffer Manager ingests the continuous five-channel sensor stream from Machine~40 and maintains a sliding window of the most recent readings. When a dependent machine lacks a complete sensor set (e.g., missing humidity or pressure due to instrumentation cost or sensor failure), the T-GCN does not treat that machine as isolated; instead, it propagates Machine~40's encoded spatial embedding through the graph convolution layers along the outgoing edges of the adjacency graph. The resulting dependent-machine prediction is therefore a \emph{graph-influenced} forecast that blends the machine's own partial observations with the anchor's thermal and mechanical state transmitted through the learned cross-correlation structure.
