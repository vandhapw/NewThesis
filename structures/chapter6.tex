...
5  & 20  & $\sim$1{,}500  & $\sim$800    & --- & $>10\%$        & measured (varies)  & 500.00   \\
...
The empirical evidence above is consistent across synthetic, scaling, and streaming benchmarks: for the maintenance-scheduling formulation as posed --- linear objective, one-hot machine assignment, integer capacity per slot --- exact branch-and-cut MILP/CBC dominates QIEA, SQA, and the QAOA simulator at every tested fleet size. The MILP scaling extension in Table~\ref{tab:milp_largescale} shows that this dominance persists up to at least $N=150$ in well under one second, comfortably inside the 60-s Kafka cycle established in Chapter~5. A genuine quantum or quantum-inspired advantage in this domain therefore requires \emph{either} (i) problem reformulations that avoid the penalty-dominated QUBO landscape (constraint-preserving mixers such as the XY ring mixer used by QAOA, problem-specific warm starts, or hybrid decomposition that confines the QUBO to small sub-instances), \emph{or} (ii) problem regimes in which the assumptions underlying MILP's polynomial-time tractability break down --- e.g.\ non-linear damage-recovery objectives, discrete crew-routing extensions, very large fleet sizes ($N \gg 10^3$) where branch-and-cut leaves its polynomial-time regime, or coupled multi-site problems whose constraint matrix loses its block structure. Identifying the empirical boundary at which one of these conditions tips the balance toward the quantum-inspired or quantum-hardware path is left to Chapter~7 as future work.
