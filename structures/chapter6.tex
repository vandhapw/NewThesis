...
\begin{enumerate}
    ...
    when measured; the $N=7$ \texttt{ibm\_fez} submission attempted in
    this study did not return within the available IBM Quantum queue
    budget, and the noise-decay projection
    $F_{\text{est}}(N=7) \lesssim 1\%$ at $p=2$ in
    Table~\ref{tab:ibm_hardware} is the operative estimate pending a
    follow-up run with extended queue allocation.
\end{enumerate}
