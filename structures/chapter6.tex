...
\section{QUBO Formulation for Quantum Processing}
...
\begin{figure}[htbp]
    ...
    \caption{QOAO Circuit.}
    \label{fig:qoao-cirquit}
\end{figure}
...
\begin{table}[htbp]
\centering
\caption{Fair comparison with all machines needing maintenance. SQA skipped for N > 10 due to timeout. Per-machine cost ratio: QIEA averages 1.5-2.2x MILP cost.}
\label{tab:scaling}
\renewcommand{\arraystretch}{1.2}
...
\vspace{0.5em}
\footnotesize
\textit{Note: Gap is computed as $(\text{Cost}_{\text{method}} - \text{Cost}_{\text{MILP}})/\text{Cost}_{\text{MILP}} \times 100\%$. 
Dashes (---) indicate that the method was not executed at that scale.}
...
\begin{figure}[htbp]
    ...
    \begin{subfigure}[t]{0.48\textwidth}
        ...
        \caption{Gap Optimality}
        \label{fig:gap_optimality}
    \end{subfigure}

    \caption{The scaling trend of solution quality, and presents the optimality gap alongside the number of time options.}
...
\begin{figure}[htbp]
    ...
    \caption{Machine Cost Analysis.}
    \label{fig:macine-cost}
\end{figure}
