...
\end{tabular}
\par\vspace{2pt}
\footnotesize\textit{Note: SQA skipped for $N > 10$ due to timeout. Gap $= (\mathrm{Cost}_{\mathrm{method}} - \mathrm{Cost}_{\mathrm{MILP}})/\mathrm{Cost}_{\mathrm{MILP}} \times 100\%$. Dashes indicate method not executed at that scale.}
\end{table}
\FloatBarrier
...
\toprule
\textbf{N} & \textbf{Qubits} & \textbf{Depth (transpiled)} & \textbf{CZ gates} & \textbf{Feasibility} & \textbf{Best quantum cost} & \textbf{MILP cost} & \textbf{Gap} \\
\midrule
5  & 20  & $\sim$1{,}500  & $\sim$800    & $>10\%$        & measured        & 500.00   & varies \\
10 & 40  & $\sim$6{,}000  & $\sim$3{,}200  & marginal       & measured        & 1{,}000.00 & varies \\
...
The noise analysis reveals exponential fidelity decay with circuit depth:
\begin{align}
F_{\text{est}}(N=5)  &= (1 - 0.004)^{800}  \approx 0.041 \quad (4.1\%) \\
F_{\text{est}}(N=10) &= (1 - 0.004)^{3{,}200} \approx 2.7 \times 10^{-6} \quad (\text{negligible}).
\end{align}
This establishes $N=5$--$10$ machines as the current noise-scaling frontier for QAOA maintenance scheduling on IBM Heron hardware with $p=4$ layers. Figure~\ref{fig:noise-frontier} visualises both the circuit complexity growth and the resulting fidelity decay.
...
Across all tested scales, the solver ranking by solution quality is:
\begin{enumerate}
    \item \textbf{MILP/CBC} --- exact optimal, polynomial time for these problem sizes (6--105~ms).
    \item \textbf{QIEA} --- best quantum-inspired method, 1.5--2.2$\times$ optimal cost, feasible at all scales up to $N=30$ (35--6{,}000~ms).
    \item \textbf{QAOA/sim} --- achieves the optimum at $N=3$ ($\text{gap}=0.0\%$), limited to $N\leq 4$ by exponential memory.
    \item \textbf{SQA} --- 2.0--2.5$\times$ optimal cost where feasible, suffers from a penalty-dominated landscape causing local-minimum trapping (870--9{,}700~ms).
\end{enumerate}
QIEA outperforms SQA by 21--26\% on instances where both produce feasible solutions. The population-based search of QIEA explores many candidate solutions simultaneously through quantum rotation gates, maintaining diversity through probabilistic observation. SQA, in contrast, relies on single-spin Metropolis updates within Trotter replicas, which mix slowly in the penalty-dominated Ising landscape characteristic of constrained scheduling problems. Figure~\ref{fig:runtime-scaling} presents the runtime scaling across all methods.
...
\section{Conclusion}
This chapter presented a comprehensive evaluation of quantum and quantum-inspired approaches to maintenance-scheduling optimisation across three experimental settings: synthetic benchmarks, a unified all-methods comparison, and real-time streaming data from the multi-agent pipeline. The key findings are summarised below.

\begin{enumerate}
    \item The QUBO formulation with data-driven penalty calibration produces correct encodings, as validated by QIEA and QAOA achieving the exact MILP optimum at $N=3$ in both synthetic and streaming experiments.
    \item QIEA is the most practical quantum-inspired solver on synthetic data, achieving feasible solutions at all tested scales ($N=3$--$30$) with gaps of 53--120\% from the MILP optimum. With real-time streaming data, however, QIEA feasibility degrades significantly beyond $N=5$ owing to higher priority magnitudes that create steeper penalty landscapes.
    \item QAOA with the XY ring mixer successfully maintains one-hot feasibility through structural constraint preservation, achieving optimal solutions at $N=3$ in both synthetic and streaming experiments.
    \item Real-time streaming data produces substantially harder scheduling problems than synthetic benchmarks. Machine priorities derived from accumulated sensor anomalies (average 155--192) create QUBO landscapes with larger penalty magnitudes, causing quantum-inspired solvers to reach their infeasibility frontier at smaller scales ($N=8$ versus $N>30$ on synthetic data).
    \item Current NISQ hardware (IBM Heron~r2) supports QAOA scheduling up to approximately $N=5$--$10$ machines before noise renders the output infeasible. Circuit depth, not qubit count, is the limiting factor.
    \item Classical MILP/CBC remains the only solver achieving feasible solutions at all tested scales across both synthetic and streaming data, with consistent sub-millisecond to single-digit-millisecond solve times. This establishes the practical bar that quantum approaches must exceed for a scheduling advantage.
\end{enumerate}

These results demonstrate that the gap between quantum and classical optimisation is wider in realistic streaming deployments than synthetic benchmarks suggest. The streaming experiment reveals that penalty-dominated QUBO landscapes --- an artefact of converting constrained problems to unconstrained form --- present the primary barrier to quantum-inspired solver performance. Future work should investigate constraint-native quantum approaches (such as QAOA with the XY mixer extended to larger scales) and improved QUBO formulations with adaptive penalty schedules that respond to the distributional characteristics of streaming-derived priorities.
