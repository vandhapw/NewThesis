...
\begin{tikzpicture}[
  font=\small,
  wire/.style={thick},
  xg/.style={draw,thick,fill=green!22,minimum width=0.6cm,minimum height=0.54cm,inner sep=1pt},
  rz/.style={draw,thick,fill=blue!14,minimum width=0.72cm,minimum height=0.54cm,inner sep=1pt},
  xyb/.style={draw,thick,fill=red!14,minimum width=0.64cm,minimum height=1.0cm,inner sep=1pt},
  mg/.style={draw,thick,fill=yellow!32,minimum width=0.6cm,minimum height=0.54cm,inner sep=1pt},
  ctrl/.style={circle,fill=black,minimum size=4.5pt,inner sep=0pt},
  slbl/.style={font=\footnotesize\bfseries,align=center}
]
% --- machine-block backgrounds ---
\fill[purple!5,rounded corners] (3.5,2.16) rectangle (8.5,4.66);
\fill[cyan!6,rounded corners]  (3.5,-0.32) rectangle (8.5,2.10);
\draw[dashed,thick,purple!55,rounded corners] (3.5,2.16) rectangle (8.5,4.66);
\draw[dashed,thick,cyan!65,rounded corners]   (3.5,-0.32) rectangle (8.5,2.10);
\node[font=\scriptsize\bfseries,purple!55,rotate=-90] at (8.72,3.41) {Machine 1 block};
\node[font=\scriptsize\bfseries,cyan!65,rotate=-90]   at (8.72,0.89) {Machine 2 block};
% --- wires + qubit labels ---
\foreach \i/\y/\m/\t in {0/4.34/1/0,1/3.72/1/1,2/3.10/1/2,3/2.48/1/3,4/1.86/2/0,5/1.24/2/1,6/0.62/2/2,7/0.00/2/3}{
  \draw[wire] (3.5,\y) -- (8.5,\y);
  \node[left,font=\scriptsize] at (3.45,\y) {$q_{\i}\,(M_{\m},t_{\t})$};
}
% --- X initialisation on the t0 qubit of each machine ---
\node[xg] at (3.95,4.34) {$X$};
\node[xg] at (3.95,1.86) {$X$};
% --- R_Z single-qubit rotations (U_C) ---
\foreach \y in {4.34,3.72,3.10,2.48,1.86,1.24,0.62,0.00}{ \node[rz] at (4.70,\y) {$R_Z$}; }
% --- intra-block ZZ couplings (one-hot penalty) ---
\foreach \ya/\yb in {4.34/3.72, 3.10/2.48, 1.86/1.24, 0.62/0.00}{
  \draw[thick] (5.35,\ya) -- (5.35,\yb);
  \node[ctrl] at (5.35,\ya){}; \node[ctrl] at (5.35,\yb){};
}
% --- inter-block ZZ coupling (capacity constraint) ---
\draw[thick] (5.72,3.72) -- (5.72,1.24);
\node[ctrl] at (5.72,3.72){}; \node[ctrl] at (5.72,1.24){};
% --- XY ring mixer, staggered columns ---
\foreach \yc in {4.03,2.79,1.55,0.31}{ \node[xyb] at (6.30,\yc) {XY}; }
\foreach \yc in {3.41,0.93}{ \node[xyb] at (6.95,\yc) {XY}; }
% --- repeat indicator ---
\node[font=\large] at (7.45,3.41) {$\cdots$};
\node[font=\large] at (7.45,0.93) {$\cdots$};
% --- measurement ---
\foreach \y in {4.34,3.72,3.10,2.48,1.86,1.24,0.62,0.00}{ \node[mg] at (8.10,\y) {$M$}; }
% --- stage labels ---
\node[slbl] at (3.95,5.02) {Init};
\node[slbl] at (5.05,5.02) {$U_C(\gamma_\ell)$};
\node[slbl] at (6.62,5.02) {$U_M(\beta_\ell)$};
\node[slbl] at (8.10,5.02) {Measure};
\node[font=\footnotesize] at (6.0,-0.78) {Repeat the $U_C(\gamma_\ell)\,U_M(\beta_\ell)$ block for $\ell = 1,\ldots,p$ layers};
\end{tikzpicture}%
}
\caption{QAOA circuit for a minimal scheduling instance ($N=2$, $T=4$).}
...
\vspace*{\fill}
\begin{center}
\begin{minipage}{\dimexpr\paperheight-90mm\relax}
\centering
