...
The QAOA-XY hardware row at $N=7$ is the second measured datum
in Table~\ref{tab:extended_streaming} (the first being $N=5$ in
Table~\ref{tab:complex_streaming_n4n5}). It confirms the pattern
we hypothesised: where every classical heuristic at $N=7$ fails
with $V=8$ violations, the constraint-native QAOA-XY mixer post-
selects a schedule with only $V=2$ violations and $36\%$ lower
pure cost ($3{,}069.07$ vs $4{,}295.42$ for the best classical
heuristic GA/TS).
...
\begin{enumerate}
    ...
    The same construction applied to $N=9$ on \texttt{ibm\_fez}
    (32 qubits; transpiled depth $2{,}958$, $5{,}390$ CZ gates,
    $10{,}334$ SX gates) returned in $18.8$~s with feasibility rate
    $0.00\%$: no measurement shot satisfied both the one-hot and
    capacity constraints simultaneously. This empirically locates
    the noise-scaling frontier between $N=7$ and $N=9$ on the current
    Heron~r2 device at $p=2$ layers: the doubling of the CZ-gate
    count from $4{,}063$ at $N=7$ to $5{,}390$ at $N=9$ drives the
    estimated single-shot fidelity below the threshold required for
    XY-ring-mixer feasibility preservation. The $N=9$ hardware
    submission therefore returns no usable schedule, confirming the
    Brandao-style depth-scaling argument that practical QAOA on
    current NISQ devices is bounded by gate fidelity, not qubit
    count, and that the empirical frontier sits at $N \approx 7$--$8$
    machines for the present complex-constraint scheduling
    formulation.
\end{enumerate}
...
machines. Decoding the QAOA-XY hardware bitstring as a fourth
panel of Figure~\ref{fig:extended_gantt_n7} is reserved for a
follow-up study: the $N=7$ \texttt{ibm\_fez} submission attempted
during this study did not return within the available queue
budget, so this comparison is left to subsequent work.
