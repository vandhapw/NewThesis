...
\end{equation}
with $C_{\text{maint}}=100$, $C_{\text{risk}}=1000$. A machine with $p_i=90$ at $t_i=36$ incurs $100 + 1000 \cdot 0.9 \cdot 36/72 = 550$ versus only $100$ at $t_i=0$, so the formulation penalizes delayed scheduling of high-priority machines.
...
The full QIEA procedure is given in Algorithm~\ref{alg:qiea} (Appendix~\ref{app:algorithms}).

The rotation-gate update is the core quantum-inspired mechanism. Unlike classical genetic algorithms that rely on crossover and mutation, QIEA evolves the probability distribution over solutions. The annealing schedule on the rotation angle $\Delta\theta$ ensures broad exploration in early generations (large rotations) and fine-tuning in later generations (small rotations), analogous to the temperature schedule in simulated annealing but operating in the amplitude space.
...
The full hardware execution pipeline is given in Algorithm~\ref{alg:qaoa_hardware} (Appendix~\ref{app:algorithms}).

Algorithm~\ref{alg:qaoa_hardware} addresses the key practical challenge of QAOA on NISQ hardware: the variational optimization loop is prohibitively expensive when executed on-device. The parameter-transfer strategy optimizes the angles on a classically simulable sub-problem, then applies them to the full-size circuit. This assumes concentration of QAOA parameters --- that optimal angles for small instances generalize to larger instances of the same problem class \cite{Brandao2018}. The transpilation step maps the abstract circuit to the hardware's native gate set and qubit connectivity, introducing SWAP gates that increase circuit depth.
Figure~\ref{fig:qaoa-circuit} illustrates the circuit architecture for a minimal instance ($N=2$ machines, $T=4$ time options), showing the initial $X$-gate preparation, the alternating problem unitary $U_C(\gamma_\ell)$ (single-qubit $R_Z$ and two-qubit $ZZ$ couplings) and XY ring-mixer $U_M(\beta_\ell)$, and the final measurement. Each machine's four qubits form an independent block within the mixer, so the one-hot feasibility of the assignment is preserved by construction. The measured cost expectation $\langle H_C\rangle$ drives a classical COBYLA optimizer with multiple restarts, which updates the variational angles $(\gamma_\ell,\beta_\ell)$ and closes the hybrid quantum--classical loop.
...
\begin{tikzpicture}[
  font=\small,
  wire/.style={thick},
  xg/.style={draw,thick,fill=pallight!55,minimum width=0.62cm,minimum height=0.56cm,inner sep=1pt},
  rzg/.style={draw,thick,fill=palmed!22,minimum width=0.80cm,minimum height=0.56cm,inner sep=1pt},
  xyg/.style={draw,thick,fill=paldark!15,minimum width=0.80cm,inner sep=1pt},
  mg/.style={draw,thick,fill=pallight!85,minimum width=0.62cm,minimum height=0.58cm,inner sep=1pt},
  zd/.style={circle,fill=paldark,minimum size=5pt,inner sep=0pt},
  zo/.style={circle,fill=palmed,minimum size=5pt,inner sep=0pt},
  barr/.style={dashed,gray!60,thick},
  stg/.style={font=\footnotesize\bfseries,align=center},
  box/.style={draw,thick,rounded corners,align=center}
]
% ===== machine-block backgrounds (one-hot encoding: T qubits per machine) =====
\fill[pallight!22,rounded corners] (0.55,3.55) rectangle (9.20,7.45);
\fill[palmed!15,rounded corners]   (0.55,-0.45) rectangle (9.20,3.45);
\node[font=\scriptsize\bfseries,paldark,rotate=90]        at (-1.95,5.5) {Machine 1 block};
\node[font=\scriptsize\bfseries,palmed!85!black,rotate=90] at (-1.95,1.5) {Machine 2 block};
% ===== qubit wires + labels =====
\foreach \i/\y/\m/\t in {0/7/1/0,1/6/1/1,2/5/1/2,3/4/1/3,4/3/2/0,5/2/2/1,6/1/2/2,7/0/2/3}{
  \draw[wire] (0.55,\y) -- (10.05,\y);
  \node[left,font=\scriptsize] at (0.48,\y) {$q_{\i}\;(M_{\m},t_{\t})$};
}
% ===== Init: X on the t0 qubit of each block -> one-hot feasible start =====
\node[xg] at (1.05,7) {$X$};
\node[xg] at (1.05,3) {$X$};
\draw[barr] (1.55,-0.5) -- (1.55,7.5);
% ===== U_C: single-qubit R_Z(gamma) =====
\foreach \y in {7,6,5,4,3,2,1,0}{ \node[rzg] at (2.30,\y) {$R_Z$}; }
% ===== U_C: intra-block ZZ couplings (one-hot penalty, every slot pair) =====
\foreach \x/\ya/\yb in {2.95/7/6,3.40/5/4,3.85/6/5,4.30/7/5,4.75/6/4,5.20/7/4,%
                        2.95/3/2,3.40/1/0,3.85/2/1,4.30/3/1,4.75/2/0,5.20/3/0}{
  \draw[thick,paldark] (\x,\ya) -- (\x,\yb);
  \node[zd] at (\x,\ya){}; \node[zd] at (\x,\yb){};
}
% ===== U_C: inter-block ZZ (machines competing for a shared time slot) =====
\foreach \ya/\yb in {7/3,6/2}{
  \draw[thick,palmed] (5.75,\ya) -- (5.75,\yb);
  \node[zo] at (5.75,\ya){}; \node[zo] at (5.75,\yb){};
}
% ===== U_M: XY ring mixer -- adjacent slot pairs =====
\foreach \yc in {6.5,4.5,2.5,0.5}{ \node[xyg,minimum height=1.16cm] at (6.55,\yc) {XY}; }
\foreach \yc in {5.5,1.5}{ \node[xyg,minimum height=1.16cm] at (7.45,\yc) {XY}; }
% ===== U_M: XY ring closure (wrap-around q3-q0 and q7-q4) =====
\foreach \ya/\yb in {7/4,3/0}{
  \draw[thick,paldark] (8.45,\ya) -- (8.45,\yb);
  \node[xyg,minimum height=0.56cm] at (8.45,\ya) {XY};
  \node[xyg,minimum height=0.56cm] at (8.45,\yb) {XY};
}
\draw[barr] (9.15,-0.5) -- (9.15,7.5);
% ===== Measurement =====
\foreach \y in {7,6,5,4,3,2,1,0}{
  \node[mg] at (9.70,\y) {};
  \draw[thick] (9.55,\y-0.10) arc (200:-20:0.155);
  \draw[thick,->] (9.70,\y-0.10) -- (9.83,\y+0.13);
}
% ===== classical register =====
\draw[thick,double,double distance=1.3pt] (9.70,0) -- (9.70,-1.35) -- (10.70,-1.35);
\draw[thick] (9.60,-0.80) -- (9.80,-0.62);
\node[font=\scriptsize] at (9.97,-0.74) {$8$};
\node[right,font=\scriptsize] at (10.72,-1.35) {classical register $c$};
% ===== stage labels =====
\node[stg] at (1.05,7.95) {Init};
\node[stg] at (3.95,7.95) {$U_C(\gamma_\ell)$};
\node[stg] at (7.50,7.95) {$U_M(\beta_\ell)$};
\node[stg] at (9.70,7.95) {Measure};
% ===== one-layer span + repeat indicator =====
\draw[thick] (1.95,-0.95) -- (1.95,-1.18) -- (9.05,-1.18) -- (9.05,-0.95);
\node[font=\footnotesize] at (5.5,-1.52)
  {one QAOA layer $U_M(\beta_\ell)\,U_C(\gamma_\ell)$ --- repeated for $\ell = 1,\dots,p$};
% ===== classical optimiser: the hybrid quantum--classical loop =====
\node[box,fill=paldark!10,minimum width=8.6cm,minimum height=1.0cm] (opt) at (4.85,-2.95)
  {\textbf{Classical optimiser} --- COBYLA with multiple restarts: minimise $\langle H_C\rangle$};
\draw[thick,->] (10.70,-1.35) |- (opt.east)
  node[pos=0.30,right=1pt,font=\scriptsize] {measured $\langle H_C\rangle$};
\draw[thick,->] (opt.north -| 2.30,0) -- (2.30,-0.55)
  node[pos=0.20,right=1pt,font=\scriptsize] {updated $(\gamma_\ell,\beta_\ell)$};
\end{tikzpicture}%
}
\caption{QAOA circuit with XY ring mixer for a minimal scheduling instance ($N=2$, $T=4$); dark and mid-tone dots mark intra- and inter-block $ZZ$ couplings.}
...
Panel~(\subref{fig:noise-frontier-a}) plots the transpiled circuit depth and the
two-qubit CZ-gate count against fleet size $N$ on a logarithmic scale; both grow
super-linearly and cross the device coherence budget --- the dotted $T_2$ limit
at $\approx 5{,}000$ --- near $N=10$. Panel~(\subref{fig:noise-frontier-b})
reports the resulting estimated whole-circuit fidelity, which decays
exponentially with $N$: from $4.1\%$ at $N=5$ (the only bar within a usable
range) to $2.7\times10^{-4}\%$ at $N=10$, and effectively zero beyond. The two
panels jointly fix $N\approx5$ as the practical hardware frontier --- panel~(a)
gives the cause (depth and entangling-gate count exceeding the coherence
envelope) and panel~(b) the consequence (fidelity falling below any recoverable
threshold).

\input{tables/ibm-extended}
...
\begin{figure}[!htbp]
    \centering
    \begin{subfigure}[b]{0.49\textwidth}
        \includegraphics[width=\textwidth]{figures/images/streaming_synth_a.png}
        \caption{Solution cost: streaming vs.\ synthetic.}
        \label{fig:streaming-vs-synthetic-a}
    \end{subfigure}\hfill
    \begin{subfigure}[b]{0.49\textwidth}
        \includegraphics[width=\textwidth]{figures/images/streaming_synth_b.png}
        \caption{Streaming-derived machine states.}
        \label{fig:streaming-vs-synthetic-b}
    \end{subfigure}
...
Figure~\ref{fig:streaming-vs-synthetic} illustrates these findings.
Panel~(\subref{fig:streaming-vs-synthetic-a}) compares the pure scheduling cost
of MILP and QIEA under streaming-derived and synthetic priorities: at equal
fleet size the streaming curves lie well above their synthetic counterparts, and
QIEA on streaming data crosses into infeasibility (red crosses) from $N\geq8$,
far earlier than on synthetic instances. Panel~(\subref{fig:streaming-vs-synthetic-b})
explains the cause --- the average priority score derived from streaming stays
uniformly high ($\approx 144$--$192$, against the $20$--$100$ synthetic range)
while the health index remains low ($\approx 0.45$) and the damage index high
($\approx 0.45$--$0.52$) across all $N$, so the entire streamed fleet sits in a
persistently degraded, all-critical state. MILP remains the only solver
achieving feasible solutions at all scales in the streaming scenario.
...
\textbf{Empirical result.} Table~\ref{tab:cs_n45} reports the head-to-head outcome on the $N=4$ and $N=5$ streaming-derived instances under the full constraint set (C1)--(C4). At $N=4$ the problem is borderline feasible (MILP returns the proven optimum at cost $1{,}336.67$); at $N=5$, with risk-horizon $\rho = 1$ slot and capacity $C = 3$, the problem becomes \emph{structurally infeasible} (5 CRITICAL machines all need slot 0, only 3 fit).
% and the question shifts from ``what is the cheapest schedule'' to ``which solver returns the most operationally usable schedule given the infeasibility''.
...
Pushing the streaming fleet past $N=5$ confirms the feasibility cliff. Table~\ref{tab:ext_str} runs the full solver suite on fresh streaming instances at $N\in\{7,9,10\}$ under (C1)--(C4): MILP reports infeasibility, and QIEA/SA/GA/TS return violation-laden best-efforts whose violation counts grow with the number of CRITICAL machines contending for the $\rho=1$-slot horizon at capacity $C=3$. The full solver suite is reported in Table~\ref{tab:ext_str}, presented together with the hardware-scaling overview of Table~\ref{tab:ibm_hw} on the shared landscape page above.
