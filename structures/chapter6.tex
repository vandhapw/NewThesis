...
\begin{enumerate}
    ...
    when measured; submissions targeting both \texttt{ibm\_fez} and
    \texttt{ibm\_kingston} backends for $N=7$ were attempted in this
    study and did not return within the available IBM Quantum queue
    budget, so the noise-decay projection
    $F_{\text{est}}(N=7) \lesssim 1\%$ at $p=2$ in
    Table~\ref{tab:ibm_hardware} remains the operative estimate
    pending a follow-up run with extended queue allocation.
\end{enumerate}
